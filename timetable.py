...
class Timetable:
	...
	def addSession(self, day, timeslot, tutor, module, sessionType):
		if day not in self.schedule:
			...
			if timeslot == 0 or timeslot > 5:
				raise ValueError("timeslot can only be: 1 or 2 or 3 ")
			else:
	...
	def task1Checker(self, tutorList, moduleList):
		...
		for day in self.schedule:
			...
			if len(dayList) != 5:
				print(str(day) + ' ' + str(len(dayList)) + " does not have every slot assigned.")
				return False
			tutorsToday = list()
	...
	def task23Checker(self, tutorList, moduleList):
		...
		for day in self.schedule:
			...
			if len(dayList) != 2:
				print(str(day) + " does not have every slot assigned.xx")
				return False
